...
root@H1BGW:~# cat /etc/firewall/iptables-save.sh
#!/bin/sh
#iptables -F
...
$IPT -A INPUT  -m state --state INVALID -j _LOG_DROP_
$IPT -A OUTPUT -m state --state INVALID -j LOG --log-prefix "IPT-LogINVALID-output: "
$IPT -A OUTPUT -m state --state INVALID -j _LOG_DROP_
...
# Christmas tree packets
$IPT -A FORWARD -p tcp -m tcp --tcp-flags ALL FIN,PSH,URG -j _LOG_DROP_
#
# Invalid TCP packets
# New incoming TCP connection packets without SYN flag set
$IPT -A FORWARD -p tcp ! --syn -m state --state NEW -j _LOG_DROP_
#
# New state packet with SYN,ACK set
$IPT -A FORWARD -p tcp -m tcp --tcp-flags SYN,ACK SYN,ACK -m state --state NEW -j _LOG_DROP_
#
# TCP packets with SYN,FIN flag set. SYN and FIN are both set.
$IPT -A FORWARD -p tcp -m tcp --tcp-flags SYN,FIN SYN,FIN -j _LOG_DROP_
#
# SYN and RST are both set.
$IPT -A FORWARD -p tcp --tcp-flags SYN,RST SYN,RST -j _LOG_DROP_
#
# FIN and RST are both set.
$IPT -A FORWARD -p tcp --tcp-flags FIN,RST FIN,RST -j _LOG_DROP_
#
# FIN is the only bit set, without the expected accompanying ACK.
$IPT -A FORWARD -p tcp --tcp-flags ACK,FIN FIN -j _LOG_DROP_
#
# PSH is the only bit set, without the expected accompanying ACK.
$IPT -A FORWARD -p tcp --tcp-flags ACK,PSH PSH -j _LOG_DROP_
#
# URG is the only bit set, without the expected accompanying ACK.
$IPT -A FORWARD -p tcp --tcp-flags ACK,URG URG -j _LOG_DROP_
#
# Null packets. All of the bits are cleared.
$IPT -A FORWARD -p tcp -m tcp --tcp-flags ALL NONE -j _LOG_DROP__LOG_DROP_
#
# Drop spoofing packets coming on the WAN interface
$IPT -A FORWARD -i $WAN_IF -s $LPB_SN -j _LOG_DROP_
$IPT -A FORWARD -i $WAN_IF -s $CLS_A -j _LOG_DROP_
$IPT -A FORWARD -i $WAN_IF -s $CLS_B -j _LOG_DROP_
$IPT -A FORWARD -i $WAN_IF -s $CLS_C -j _LOG_DROP_
$IPT -A FORWARD -i $WAN_IF -s $CLS_D_MLTCST -j _LOG_DROP_
$IPT -A FORWARD -i $WAN_IF -s $CLS_E_RESNET -j _LOG_DROP_
$IPT -A FORWARD -i $WAN_IF -s $BCST_SRC_NET -j _LOG_DROP_
$IPT -A FORWARD -i $WAN_IF -s $LINK_LCL -j _LOG_DROP_
$IPT -A FORWARD -i $WAN_IF -s $TEST_NET -j _LOG_DROP_
$IPT -A FORWARD -i $WAN_IF -s $WAN_IP -j _LOG_DROP_
...
#
$IPT -A INPUT -j _LOG_DROP_
#
# OUTPUT chain close rules.
$IPT -A OUTPUT -j _LOG_DROP_
#
# FORWARD chain close rules.
$IPT -A FORWARD -j _LOG_DROP_
#
...
# rdp
# ssh
$IPT -t nat -A PREROUTING  -i enp0e9 -s 192.168.192.0/24 \
...
# WAN Input Chain
#
$IPT -A INPUT -s 192.168.192.100 -i enp0s9 -p tcp -d 192.168.92.105 -j _WAN_IN_
$IPT -A INPUT -i enp0s9 -p tcp -j _WAN_IN_
$IPT -A INPUT -s 192.168.192.100 -i enp0s9 -p udp -d 192.168.92.105 -j _WAN_IN_
$IPT -A INPUT -i enp0s9 -p udp -j _WAN_IN_
#
$IPT -A _WAN_IN_ -m conntrack --ctstate NEW -j LOG --log-prefix="IPT-New: _WAN_IN_ "
#
$IPT -A _WAN_IN_ -p tcp --match multiport --sports 1025:65535 \
                     --match multiport --dports 2221,25,465,995,993,443,80 \
                     -m conntrack --ctstate NEW,ESTABLISHED \
                     -i enp0s9 -o enp0s8 -j ACCEPT
#
$IPT -A _WAN_IN_ -p tcp --match multiport --sports 1025:65535 \
                     --match multiport --dports 2222 \
                     -m conntrack --ctstate NEW,ESTABLISHED \
                     -i enp0s9 -o enp0s10 -j ACCEPT
#
$IPT -A _WAN_IN_ -p tcp --match multiport --sports 2221,25,465,995,993,443,80 \
                     --match multiport --dports 1025:65535 \
                     -m conntrack --ctstate ESTABLISHED \
                     -i enp0s8 -o enp0s9 -j ACCEPT
#
$IPT -A _WAN_IN_ -p tcp --match multiport --sports 2222 \
                     --match multiport --dports 1025:65535 \
                     -m conntrack --ctstate ESTABLISHED \
                     -i enp0s10 -o enp0s9 -j ACCEPT
#
$IPT -A _WAN_IN_ -s 192.168.192.100/24 -p tcp \
                     --match multiport --sports 1025:65535 \
                     --match multiport --dports 22,2221:2225,8020:8100,5900:5940,3389 \
                     -m conntrack --ctstate NEW,ESTABLISHED -i enp0s9 -j ACCEPT
#
$IPT -A _WAN_IN_ -s 192.168.120.0/24 -p tcp \
                     --match multiport --sports 22,2221:2225,8020:8100,5900:5940,3389 \
                     --match multiport --dports 1025:65535 \
                     -m conntrack --ctstate ESTABLISHED -i enp0s8 -o enp0s9 -j ACCEPT
#
# WAN_to_LOGGING and DROP
iptables -A WAN_INPUT -j _LOG_DROP_



########################################################################
# INPUT chain, DMZ traffic going out to WAN or INTernal segments behind INT_GW.
# - Servers answers traffic,
# - Little bit rules for diagnostical and debugging steps (should be disables after),
# - etc.
# _DMZ_OUT_
#$IPT -t filter -A INPUT -i $DMZ_IF -s $DMZ_SN -p tcp \
#               --dport $UNPR_PRTS \
#               -m multiport --sports $SSH_DMZP,$SSH_STDP \
#               -m state --state ESTABLISHED,RELATED -j _DMZ_OUT_
#
#$IPT -t filter -A INPUT -i $DMZ_IF -s $DMZ_SN \
#                -p icmp --icmp-type echo-request -m state --state NEW -j _DMZ_OUT_
#
#$IPT -t filter -A INPUT -i $WAN_IF -p icmp --icmp-type echo-reply \
#                -m conntrack --ctstate ESTABLISHED,RELATED -j _DMZ_OUT_
#
#$IPT -A _DMZ_OUT_ -j ACCEPT
...
#
# Null packets. All of the bits are cleared.
#$IPT -A FORWARD -p tcp -m tcp --tcp-flags ALL NONE -j _LOG_DROP_
#
# Drop spoofing packets coming on the WAN interface
#$IPT -A FORWARD -i $WAN_IF -s $LPB_SN -j _LOG_DROP_
#$IPT -A FORWARD -i $WAN_IF -s $CLS_A -j _LOG_DROP_
...
########################################################################
# FORWARD RULES and CHAINS:
# _FWD_IN_  -- FORWARD TO INTERNAL
# _FWD_DMZ_ -- FORWARD OUT FROM DMZ
# _FWD_OUT_ -- FORWARD OUT FROM INTERNAL ZONES
#
#$IPT -t filter -A FORWARD -i $WAN_IF -o $DMZ_IF -p tcp \
#                           --sport $UNPR_PRTS \
#                           --match multiport --dports $SSH_DMZP \
#                           -d $DMZ_SN -m conntrack --ctstate NEW,RELATED \
#                           -j _FWD_IN_
#
#$IPT -t filter -A FORWARD -i $WAN_IF -o $INT_IF -p tcp \
#                           --sport $UNPR_PRTS \
#                           --match multiport --dports $SSH_INTP \
#                           -d $INT_SN -m conntrack --ctstate NEW,RELATED \
#                           -j _FWD_IN_
#
#$IPT -t filter -A FORWARD -i $DMZ_IF -o $WAN_IF -p tcp \
#                           --dport $UNPR_PRTS -s $DMZ_SN \
#                           --match multiport --sports $SSH_DMZP \
#                           -m conntrack --ctstate ESTABLISHED,RELATED \
#                           -j _FWD_DMZ_

#
#$IPT -t filter -A FORWARD -i $INT_IF -o $WAN_IF -p tcp \
#                           --dport $UNPR_PRTS -s $INT_SN \
#                           --match multiport --sports $SSH_INTP \
#                           -m conntrack --ctstate ESTABLISHED,RELATED \
#                           -j _FWD_OUT_
#
#$IPT -A _FWD_IN_ -j ACCEPT
#$IPT -A _FWD_DMZ_ -j ACCEPT
#$IPT -A _FWD_OUT_ -j ACCEPT
...
# PREROUTING chain.
# DNAT MAPPING LIST:
#iptables -t nat -A PREROUTING  -i enp0e3 \
#                 -s 192.168.111.220 -d 192.168.111.130 \
#                 -p tcp --dport 8011 -j DNAT \
#                 --to-destination 192.168.10.11:8080
#iptables -t nat -A PREROUTING  -i $WAN_IF -p tcp \
#                 -d $SSH_DMZsshIP \
#                 --dport $SSH_DMZP -j DNAT \
#                 --to-destination $SSH_DMZsshIP:$SSH_DMZP

#iptables -t nat -A PREROUTING  -i $WAN_IF -p tcp \
#                 -d $SSH_INTsshIP \
#                 --dport $SSH_INTP -j DNAT \
#                 --to-destination $SSH_INTsshIP:$SSH_INTP

########################################################################
# POSTROUTING chain.
#
#iptables -t nat -A POSTROUTING -o $WAN_IF -s $DMZ_SN -p ALL -j SNAT --to $WAN_IP
#iptables -t nat -A POSTROUTING -o $WAN_IF -s $INT_SN -p ALL -j SNAT --to $WAN_IP
#
########################################################################
# INPUT chain close rules.
#
...
Where is SSH anti-BRUTE FORCE rules?
